package com.ramzmania.aicammvd.data.moshiFactories
...
internal class MyKotlinJsonAdapter<T>(
    ...
    override fun fromJson(reader: JsonReader): T {
        ...
        ////////////////// Crash Fix - Expected BEGIN_OBJECT but was STRING //////////////////////////////////
        reader.beginObject()
        ...
        override val entries: Set<Map.Entry<KParameter, Any?>>
            get() {
                val allPossibleEntries = parameterKeys.mapIndexed { index, value ->
                    java.util.AbstractMap.SimpleEntry<KParameter, Any?>(
                        value,
                        parameterValues[index]
                    )
                }
                return allPossibleEntries.filterTo(LinkedHashSet<Map.Entry<KParameter, Any?>>()) {
...
class MyKotlinJsonAdapterFactory : JsonAdapter.Factory {
    override fun create(type: Type, annotations: MutableSet<out Annotation>, moshi: Moshi)
        ...
        for (property in rawTypeKotlin.memberProperties) {
            ...
            val resolvedPropertyType = Util.resolve(type, rawType, property.returnType.javaType)
